...
func (server *Server) requestDataHandler(ctx context.Context, req *pb.MessageRequest) (*pb.MessageResponse, error) {
	...
	if req.Meta.OID > 0 && reqName == SellerOrderList {
		return server.sellerGetOrderByIdHandler(ctx, req.Meta.PID, req.Meta.OID, filterValue)
	}
...
func (server *Server) operatorOrderListHandler(ctx context.Context, oid uint64, filter FilterValue, page, perPage uint32,
	...
	if oid > 0 {
		orderFilter = func() (interface{}, string, int) {
			return bson.D{{"orderId", oid}, {"deletedAt", nil}, {"packages.subpackages.state.name", server.operatorFilterState[filter].StateName()}},
				sortName, sortDirect
		}
	} else {
		orderFilter = func() (interface{}, string, int) {
			return bson.D{{"deletedAt", nil}, {"packages.subpackages.state.name", server.operatorFilterState[filter].StateName()}},
				sortName, sortDirect
		}
	}
	...
	for i := 0; i < len(orderList); i++ {
		...
		for j := 0; j < len(orderList[i].Packages); j++ {
			for z := 0; z < len(orderList[z].Packages[j].Subpackages); z++ {
				for t := 0; t < len(orderList[z].Packages[j].Subpackages[z].Items); t++ {
					order.BasketSize += orderList[z].Packages[j].Subpackages[z].Items[t].Quantity
				}
			}
		}
...
func (server *Server) operatorOrderDetailHandler(ctx context.Context, oid uint64) (*pb.MessageResponse, error) {
	...
	subpackageList := make([]*pb.OperatorOrderDetail_Subpackage, 0, 32)
		...
		for j := 0; j < len(order.Packages[i].Subpackages); j++ {
			...
			for x := 0; x < len(order.Packages[i].Subpackages[j].Tracking.History); x++ {
				state := &pb.OperatorOrderDetail_Subpackage_StateHistory{
					...
					Index:     order.Packages[i].Subpackages[j].Tracking.History[x].Index,
					UTP:       order.Packages[i].Subpackages[j].Tracking.History[x].Actions[0].Type,
					CreatedAt: "",
				}
			}
...
func (server *Server) sellerGetOrderHandler(ctx context.Context, oid uint64, pid uint64, filter FilterValue, page, perPage uint32,
	sortName string, direction SortDirection) (*pb.MessageResponse, error) {
