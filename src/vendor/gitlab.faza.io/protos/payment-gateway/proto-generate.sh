...
if `echo `protoc -I=. --go_out=plugins=grpc:. $FILE.proto``; then
    echo `mv $FILE.pb.go $FILE.go`
    echo "Generated the proto file. $DIR for the generated file."
    if [[ $2 != "--skip-tag" ]]; then
        echo `protoc-go-inject-tag -input=$DIR$FILE.go`
        echo "Generated tags for the $DIR$FILE.pb.go"
    fi;
    exit 0
else
    echo "Failed to generate proto file."
    exit 1
fi
