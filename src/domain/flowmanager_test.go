...
func TestFlowManagerSteps(t *testing.T) {
	...
	if err := stepValidation(GetIndexStepsMap()[1], 1, "1.New_Order_Failed", []int{}); err != nil {
		t.Fatalf("validate step1 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[10], 10, "10.Payment_Pending", []int{11, 12}); err != nil {
		t.Fatalf("validate step10 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[12], 12, "12.Payment_Failed", []int{}); err != nil {
		t.Fatalf("validate step12 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[11], 11, "11.Payment_Success", []int{20,14}); err != nil {
		t.Fatalf("validate step11 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[14], 14, "14.Payment_Rejected", []int{80}); err != nil {
		t.Fatalf("validate step14 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[20], 20, "20.Seller_Approval_Pending", []int{30,21}); err != nil {
		t.Fatalf("validate step20 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[21], 21, "21.Shipment_Rejected_By_Seller", []int{80}); err != nil {
		t.Fatalf("validate step21 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[30], 30, "30.Shipment_Pending", []int{31,33}); err != nil {
		t.Fatalf("validate step30 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[31], 31, "31.Shipped", []int{32,34}); err != nil {
		t.Fatalf("validate step31 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[32], 32, "32.Shipment_Delivered", []int{40,41,43}); err != nil {
		t.Fatalf("validate step32 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[33], 33, "33.Shipment_Detail_Delayed", []int{31,36}); err != nil {
		t.Fatalf("validate step33 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[34], 34, "34.Shipment_Delivery_Pending", []int{32,35}); err != nil {
		t.Fatalf("validate step34 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[35], 35, "35.Shipment_Delivery_Delayed", []int{32,36}); err != nil {
		t.Fatalf("validate step35 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[36], 36, "36.Shipment_Canceled", []int{80}); err != nil {
		t.Fatalf("validate step36 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[40], 40, "40.Shipment_Success", []int{90}); err != nil {
		t.Fatalf("validate step40 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[41], 41, "41.Return_Shipment_Pending", []int{42,44}); err != nil {
		t.Fatalf("validate step41 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[42], 42, "42.Return_Shipped", []int{50,51}); err != nil {
		t.Fatalf("validate step42 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[43], 43, "43.Shipment_Delivery_Problem", []int{40,41}); err != nil {
		t.Fatalf("validate step43 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[44], 44, "44.Return_Shipment_Detail_Delayed", []int{40,42}); err != nil {
		t.Fatalf("validate step44 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[50], 50, "50.Return_Shipment_Delivered", []int{53,55}); err != nil {
		t.Fatalf("validate step50 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[51], 51, "51.Return_Shipment_Delivery_Pending", []int{50,52}); err != nil {
		t.Fatalf("validate step51 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[52], 52, "52.Return_Shipment_Delivery_Delayed", []int{50,54}); err != nil {
		t.Fatalf("validate step52 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[53], 53, "53.Return_Shipment_Delivery_Problem", []int{54,55}); err != nil {
		t.Fatalf("validate step53 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[54], 54, "54.Return_Shipment_Canceled", []int{90}); err != nil {
		t.Fatalf("validate step54 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[55], 55, "55.Return_Shipment_Success", []int{80}); err != nil {
		t.Fatalf("validate step55 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[80], 80, "80.Pay_To_Buyer", []int{81,82}); err != nil {
		t.Fatalf("validate step80 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[81], 81, "81.Pay_To_Buyer_Success", []int{}); err != nil {
		t.Fatalf("validate step81 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[82], 82, "82.Pay_To_Buyer_Failed", []int{81}); err != nil {
		t.Fatalf("validate step82 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[90], 90, "90.Pay_To_Seller", []int{91,92}); err != nil {
		t.Fatalf("validate step90 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[91], 91, "91.Pay_To_Seller_Success", []int{93}); err != nil {
		t.Fatalf("validate step91 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[92], 92, "92.Pay_To_Seller_Failed", []int{91}); err != nil {
		t.Fatalf("validate step92 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[93], 93, "93.Pay_To_Market", []int{94,95}); err != nil {
		t.Fatalf("validate step93 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[94], 94, "94.Pay_To_Market_Success", []int{}); err != nil {
		t.Fatalf("validate step94 failed: %s\n", err)
	}

	if err := stepValidation(GetIndexStepsMap()[95], 95, "95.Pay_To_Market_Failed", []int{94}); err != nil {
