...
func (state returnDeliveryPendingState) Process(ctx context.Context, iFrame frame.IFrame) {
	if iFrame.Header().KeyExists(string(frame.HeaderSubpackage)) {
		...
		if event.EventType() == events.Action {
			...
			var returnDeliveryPendingAction *entities.Action
										...
										if actionItem.InventoryId == pkgItem.Subpackages[i].Items[j].InventoryId {
											...
											if actionItem.Quantity != pkgItem.Subpackages[i].Items[j].Quantity {
												if newSubPackage == nil {
													...
													returnDeliveryPendingAction = &entities.Action{
			...
			if newSubPackage != nil {
				...
				state.UpdateSubPackage(ctx, newSubPackage, returnDeliveryPendingAction)
