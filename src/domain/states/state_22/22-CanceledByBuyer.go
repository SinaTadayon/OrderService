...
func (state canceledByBuyerState) Process(ctx context.Context, iFrame frame.IFrame) {
	if iFrame.Header().KeyExists(string(frame.HeaderSubpackage)) {
		...
		if err != nil {
			...
			logger.Audit("Cancel by Buyer success, orderId: %d, sellerId: %d, itemId: %d", pkgItemUpdated.OrderId, pkgItemUpdated.SellerId)
...
func (state canceledByBuyerState) releasedStockPackage(ctx context.Context, pkgItem *entities.PackageItem) error {
