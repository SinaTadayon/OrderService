...
func (notificationState notificationActionLauncher) doUpdateOrderState(ctx context.Context, order *entities.Order, index int,
	acceptedAction actions.IEnumAction, result bool, reason string) {
	order.Items[index].OrderStep.CurrentState.Name = notificationState.Name()
	order.Items[index].OrderStep.CurrentState.Index = notificationState.Index()
	order.Items[index].OrderStep.CurrentState.Type = notificationState.Actions().ActionType().Name()
