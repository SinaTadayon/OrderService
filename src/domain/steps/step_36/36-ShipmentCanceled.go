...
func New(childes, parents []steps.IStep, orderRepository repository.IOrderRepository,
	...
	return &shipmentCanceledStep{steps.NewBaseStep(stepName, stepIndex, orderRepository,
...
func NewOf(name string, index int, orderRepository repository.IOrderRepository,
	...
	return &shipmentCanceledStep{steps.NewBaseStep(name, index, orderRepository,
		itemRepository, childes, parents, states)}
}
...
func (shipmentCanceled shipmentCanceledStep) ProcessMessage(ctx context.Context, request *message.Request) promise.IPromise {
	panic("implementation required")
}

func (shipmentCanceled shipmentCanceledStep) ProcessOrder(ctx context.Context, order entities.Order) promise.IPromise {
	panic("implementation required")
}
