...
func New(childes, parents []steps.IStep, orderRepository repository.IOrderRepository,
	...
	return &paymentFailedStep{steps.NewBaseStep(stepName, stepIndex, orderRepository,
...
func NewOf(name string, index int, orderRepository repository.IOrderRepository,
	...
	return &paymentFailedStep{steps.NewBaseStep(name, index, orderRepository,
		itemRepository, childes, parents, states)}
}
...
func (paymentFailed paymentFailedStep) ProcessOrder(ctx context.Context, order entities.Order) promise.IPromise {
	panic("implementation required")
}
