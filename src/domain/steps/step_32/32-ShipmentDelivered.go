...
func New(childes, parents []steps.IStep, orderRepository repository.IOrderRepository,
	...
	return &shipmentDeliveredStep{steps.NewBaseStep(stepName, stepIndex, orderRepository,
...
func NewOf(name string, index int, orderRepository repository.IOrderRepository,
	...
	return &shipmentDeliveredStep{steps.NewBaseStep(name, index, orderRepository,
		itemRepository, childes, parents, states)}
}
...
func (shipped shipmentDeliveredStep) ProcessMessage(ctx context.Context, request *message.Request) promise.IPromise {
	panic("implementation required")
}

func (shipped shipmentDeliveredStep) ProcessOrder(ctx context.Context, order entities.Order) promise.IPromise {
	panic("implementation required")
}
