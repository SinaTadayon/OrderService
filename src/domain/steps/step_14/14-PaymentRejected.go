...
func New(childes, parents []steps.IStep, orderRepository repository.IOrderRepository,
	...
	return &paymentRejectedStep{steps.NewBaseStep(stepName, stepIndex, orderRepository,
...
func NewOf(name string, index int, orderRepository repository.IOrderRepository,
	...
	return &paymentRejectedStep{steps.NewBaseStep(name, index, orderRepository,
		itemRepository, childes, parents, states)}
}
...
func (paymentRejected paymentRejectedStep) ProcessMessage(ctx context.Context, request *message.Request) promise.IPromise {
	panic("implementation required")
}

func (paymentRejected paymentRejectedStep) ProcessOrder(ctx context.Context, order entities.Order) promise.IPromise {
	panic("implementation required")
}
